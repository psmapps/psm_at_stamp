...
void genNewQRCode() async{
  ...
  final HttpsCallable callable = CloudFunctions.instance.getHttpsCallable(
      ...
      if (resp.data == "404"){
        Navigator.pop(context);
        showMessageBox(false, "ไม่พบแสตมปืที่คุณได้สิทธิแจก", "กรุณาติดต่อ PSM @ STAMP Team");
...
void initState(){
  ...
  Firestore.instance.collection("Stamp_Data").where("userId", isEqualTo: widget.userId).getDocuments().then((doc) {
    if(doc.documents.isNotEmpty){
      ...
      Navigator.pop(context);
      showMessageBox(false, "ไม่พบสิทธิการแจกแสตมป์ของคุณ", "ไม่พบสิทธิการแจกแสตมป์ของคุณ กรุณาติดต่อ PSM @ STAMP Team เพื่อแก้ไขปัญหาต่อไป");
    }
  });
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Flexible(
                                ...
                                child: Text("แสตมป์จากฐานกิจกรรม: " + stampName, maxLines: 3, style: TextStyle( fontSize: 15, fontWeight: FontWeight.bold, color: Colors.greenAccent),),
                              ),
                            ),
