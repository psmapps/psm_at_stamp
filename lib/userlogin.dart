        ...
        switch (result.status) {
          case AuthorizationStatus.authorized:
            ...
            FirebaseAuth.instance.currentUser().then((val) async {
              print(val.uid);
              Firestore.instance.collection("Stamp_User").document(val.uid).get().then((valfire) async{
                String accessToken = new String.fromCharCodes(appleIdCredential.authorizationCode);
                if (valfire.exists){
                  ...
                  Navigator.push(context, MaterialPageRoute(builder: (context) => createUser(displayName: "(Sign in with Apple)", userId: val.uid, profileImage: "https://firebasestorage.googleapis.com/v0/b/satitprasarnmit-psm-at-stamp.appspot.com/o/user.png?alt=media&token=eb023a2a-0d9e-46f2-8301-ef4e0e20cfee", accessToken:accessToken,)));
                }
              });
            });
          ...
          case AuthorizationStatus.error:
            print(result.error);
            Navigator.pop(context);
            showMessageBox(false, "เกิดข้อผิดพลาด", "เกิดข้อผิดพลาดขณะเข้าสู่ระบบด้วย Apple กรุณาลองใหม่อีกครั้ง");
            break;
        }
      } else {
        Navigator.pop(context);
            showMessageBox(false, "เกิดข้อผิดพลาด", "อุปกรณ์ของคุณไม่รองรับการเข้าสู่ระบบด้วย Apple (รองรับตั้งเเต่ IOS 13 เป็นต้นมา)");
      }
    }
  }
